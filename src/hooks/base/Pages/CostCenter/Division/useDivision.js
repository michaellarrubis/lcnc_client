...
const useDivision = () => {
  ...
  const dataList = useSelector(state => state.divisions.divisions.items);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const res = await getDivisionItemsService(savedPageIndex, params);
      const newList =
        savedPageIndex !== 1
          ? divisionList.concat(res.data.items)
          : res.data.items;
      ...
      setDivisionList(
        newList.map(o => {
          ...
          return {
            id: o.id,
            division_code: o.code || o.division_code,
  ...
  const divisionListMemo = useMemo(() => {
    return divisionList ?? [];
  }, [divisionList]);
  ...
  const handleDeleteModal = useCallback(
    ({ modal, id, code }) => {
      ...
      setuniqueCode(modal ? id : null);
      setUniqueCodeName(modal ? code : null);
      setOverflowStyle(modal);
    },
