...
const useSection = () => {
  ...
  const dataList = useSelector(state => state.sections.sections.items);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const res = await getSectionsService(savedPageIndex, params);
      const newList =
        savedPageIndex !== 1
  ...
  const handleDeleteModal = useCallback(
    ({ modal, id, section_code }) => {
      setShowDeleteModal(modal);
      setuniqueCode(modal ? id : null);
      setUniqueCodeName(modal ? section_code : null);
      // disable scroll when modal is shown
      setOverflowStyle(modal);
    },
